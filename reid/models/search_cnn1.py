...
class Cell(nn.Module):
    ...
    def __init__(self, in_channels, out_channels):
        ...
        # Create interaction options.
        self._op4s = nn.ModuleList()
        for primitive in PRIMITIVES:
            op = OPS[primitive](mid_channels, mid_channels)
            self._op4s.append(op)
    ...
    def forward(self, x, weights):
        ...
        x4b = self.conv4b(x3b)
        x4as, x4bs = [], []
        for op, w in zip(self._op4s, weights[1]):
            x4a_op, x4b_op = op((x4a, x4b))
            x4as.append(x4a_op * w)
            x4bs.append(x4b_op * w)
        x4a = sum(x4as)
        x4b = sum(x4bs)
...
class SearchCNN(nn.Module):
    ...
    def reset_alphas(self):
        ...
        self._arch_weights = Variable(
            1e-3 * torch.randn((18, num_ops)).cuda(), requires_grad=True
        )
